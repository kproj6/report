...
The rest endpoints is what the user sees. It is a collection of URL's that the user can point their HTTP client (browser) to, to get a representation of the resource that endpoint represent. The user can specify additional constraints on what she wants in the form of query parameters. 
...
This project uses Representational state transfer (REST) \cite{REST:elkstein}, which is an architecture style for designing network applications that uses almost exclusively HTTP protocol. As so this application has client-server architecture with stateless cacheable requests.

Despite REST supports all CRUD (\textbf{C}reate, \textbf{R}ead, \textbf{U}pdate, \textbf{D}elete) operations, this project makes use only of read operation, because the type of application does not support any modification of remote data. REST request consists of a simple HTTP GET request, where URI specifies the service and GET parameters are passed to the specific service handler. \\
...
Healthcheck (health root service) currently supports only shallow healthcheck under path - \texttt{/health/shallow}. This check returns plain text response with HTTP status 200 and text "Shallow health check ok" if the server is running and able to receieve requests. \\
...
All features support \texttt{area} subservice, which provides information about given feature in given area, currently in a form of PNG image of size 256x256 px. In case of error, HTTP status is set accordingly and JSON specifying the error is returned. Returned JSON then can contain those fields:
\begin{description}
...
\begin{description}
	...
	\item[Response] ~\\
		\begin{description}
			\item[HTTP status] \texttt{400 Bad Request}
...
\begin{description}
	...
	\item[Response] ~\\
		\begin{description}
			\item[HTTP status] \texttt{500 Server Error}
