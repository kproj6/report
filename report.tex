...
Each group member was assigned one or more areas of responsibility. The group was split into two teams to research for the \gls{pre-study}, one for \gls{front-end} and one for \gls{back-end}. This split will remain as the customer wants us to create both a custom front and \gls{back-end}.
...
\section{Work on the \gls{pre-study}}
The main goal of the first project period was to produce a preliminary study. This presented the customer with possibilities on how the work should proceed. Based on a recommendation by the group and after some discussion, the customer made a decision. The \gls{pre-study} took several weeks to produce, and work focused on these tasks:

\paragraph{Documenting the current solution}
To gain a better perspective on what the customer wanted, we documented what SINTEF's current system looked like, what data it presented and how. This project aims to solve some of the challenges and limitations of the current system.
...
\\
\noindent
This sprint marks the formal start of the implementation work. Up to this point, the group has focused mainly on the \gls{pre-study}, which has done an extensive study into the possible choices for the customer. In this process, \glspl{prototype} were implemented, but this was only meant for the group to get an idea of the work involved. Thus, the real work of implementation starts at this point, though the prototyping obviously will be a great help in the work ahead. 
...
From our formalized scope and the choice the customer made based on our \gls{pre-study}, we created a list of requirements. The list of functional requirements will serve as our project backlog. In discussion with the group, the customer gave each requirement a priority to serve as a guideline when creating the sprint backlogs.
...
The backlog for the first sprint consisted mainly of tasks that are connected to the \gls{pre-delivery}. This because of the strict deadline this delivery has, as well as the need for everyone to work on the \gls{pre-delivery} to ensure that it would be done on time.
...
We have chosen to use the quite vague goal of the customer as the only task for the \gls{front-end}. This has the consequence of setting a task that is a lot larger than one work day. Despite of this, since the task is vague, we found it difficult to split it further. For the \gls{back-end} side of the project, the tasks are much more specific.
...
\section{Sprint backlog}
This sprint is focused mostly on the \gls{back-end} work. As we will have feedback from the customer in the middle of the sprint, there might be some new tasks added to the \gls{front-end} part of the backlog. The customer has in meetings with the group said that they will probably have input on the \gls{front-end}, especially the user interface. The sprint backlog is given in table \ref{tab:Sprint3Backlog}. It can be seen that not all tasks have been estimated. Some of these tasks where deemed too difficult to estimate, while some was not estimated when they where picked by a team member in the middle of the sprint. 
...
Write sprint 2                                                                                  & 3                                                                   &                                                        & Hans Kristian \\
...
Write sprint 2                                                                                       & 3                                                                   & 4                                                      & Hans Kristian       \\
...
Write sprint 5                                        & 3                                                                   & 4                                                      & Hans Kristian \\
...
\paragraph{Converting \gls{netcdf} files}
...
\rowcolor[HTML]{C0C0C0} 
\multicolumn{4}{l}{\cellcolor[HTML]{C0C0C0}\textbf{Back-end}}                                                                                                                                                       \\
Investigate \gls{Fimex} and Proj.4                           & 4                                                                   & 2                                                       & Marco                      \\
Investigate \gls{Fimex} headers not working                  & 3                                                                   & 1                                                       & Marco                      \\
...
Write about \gls{netcdf} manager                        & 3                                                                   & 2,5                                                     & Ondrej                     \\
Write about indexing (Challenges)                      & 3                                                                   & 2,5                                                     & Emil                       \\
Write about indexing (Final product)                   & 8                                                                   & 9,5                                                     & Emil                       \\
Write glossary appendix                                & 2                                                                   & 3,5                                                     & Hans Kristian              \\
...
Write about use of \gls{Fimex}                               & 4                                                                   & 5                                                       & Marco                      \\
...
We needed to look into a \gls{netcdf} file containing all the different types of data (i.e. temperature, salinity, currents and depth), since the \gls{back-end} now reads from one \gls{netcdf} file and this can allow the \gls{front-end} to visualize all different types of data.
A \gls{netcdf} file, sizing approximately 2 GB, meeting this requirement, has been found in the Hard Drive provided by the customer. An ad-hoc \gls{Fimex} configuration script has been written and the file converted.
Technical details for this problem are explained in \ref{sec:ChallengeFimexMissingAttribute}.
...
A tile matrix is a flat grid of tiles (square images) with a given top-left coordinate, tile size, and number of tiles. It is normal to have several tile matrices for the same data-set - one tile matrix for each zoom level.  
...
This process unavoidably results in a distorted result. Several different projections exist, and some are more common or popular than others.
...
\paragraph{Proj.4}
\begin{quote}
PROJ.4\footnote{http://trac.osgeo.org/proj/} is a library for performing conversions between cartographic projections. The library is based on the work of Gerald Evenden at the \gls{USGS}, but is now an OSGeo project maintained by Frank Warmerdam. The library also ships with executables for performing these transformations from the command line.
\end{quote}
PROJ.4 has been placed under an MIT license. 

For the \gls{back-end} part, we have been interested in GeoTools\footnote{http://geotools.org/}, a Java \gls{GIS} library that includes sophisticated projections support.

The coordinates parameters for the REST \gls{API} exposed by the server side application are in the form of Latitude and Logitude, i.e. in \gls{GCS}(with \gls{EPSG} code "EPSG:4326"), whereas the data stored in the NetCDF file is in a parameterized Polar Stereographic Projection (\gls{EPSG} code "EPSG:9810", with parameters specified by each NetCDF file).
...
The customer provided us the Proj.4 String Notation for the Polar Stereographic Projection, with parameters, that is as follows:
\begin{lstlisting}
...
Unfortunately GeoTools does not support the Proj.4 String description. Instead, we realized that it works only with the \gls{WKT} format.
...
\end{lstlisting}
To use this in Java, first the quotes had to be escaped to avoid mistakes. 
Additionally we added the "UNIT" parameter as the last parameter, directly as a child of the PROJCS main element. The unit specified has been "m", with a conversion scale (it is a required attribute\footnote{http://www.geoapi.org/3.0/javadoc/org/opengis/referencing/doc-files/WKT.html\#UNIT}).
...
With this string as a parameter, GeoTools did not give any errors, and the conversion from PSP to LatLong has been performed correctly (a backwards conversion has been performed to check the result, as well as comparison with values provided by online conversion tools).
...
It was decided that this solution is too slow to be practical, as you have to pay the price of reprojecting the data every time it is requested. It would also have severely complicated the code: When reading a \gls{netcdf}, you can only read out a spatial rectangle that is aligned with the dataset.

When the user application requests a rectangle in a foreign projection (relative to the dataset), it is requesting what is effectively a trapezoid region of the data (in the native projection), see figure \ref{fig:tileMatrixSet}. This means that we have to read out a rectangle that contains the requested trapezoid, do a costly reprojection computation, discard the excessive data points, resample the result and then return an image of it.
...
In the scenario in which a base map is provided in the Polar Stereographic Projection (and potentially in any projection), the tiles requested by the user from \gls{netcdf} files can be directly displayed without the need of any projection transformation. This would be a straightforward solution to the projection problem.
...
Default \gls{CRS} codes known to OpenLayers currently are the following:
...
We tried to convert from the Mercator projection (EPSG:3785), in which OpenLayers displays data, to the Polar Stereographic Projection, in which data is natively stored in the \gls{netcdf}-files we are using. This way, we can hypothetically provide the parameter as input to the back-end if the API requires coordinates in Polar Stereographic Projection.
...
This code produced the following result in the JavaScript browser console:
\begin{lstlisting}
Before projecting from EPSG:3785: 911429.68975043,9364527.30573214 - 977165.5340756816,9333952.49441807 
...
A challange has been making \gls{Fimex} command line tool available in the command line, due to a lot of hard-to-configure dependencies to build and install. Binary versions are not available for most of the dependency packages, nor is a binary version of \gls{Fimex} available for other operating systems than Ubuntu.
Specifically, \gls{Fimex} requires at least the following libraries to be installed in the system before compilation:
\begin{itemize}
\item c99/c++ compiler
\item libxml2 $>=$ 2.5.0
...
We finally succeded in installing \gls{Fimex} on an Linux Ubuntu Operating System, because it was the only operating system for which a binary version of \gls{Fimex} is available, via \gls{PPA}\footnote{Using a Personal Package Archive (\gls{PPA}), it is possible to distribute software and updates directly to Ubuntu users}

\subsubsection{\gls{Fimex} Command Line Tool usage}
\label{sec:ChallangeFimexCommandLine}
\gls{Fimex} as a command line tool accepts parameters listed in its main website for the documentation\footnote{http://fimex.met.no/doc/programDoc.html}.
Generally, its usage is as follows:
\begin{lstlisting}
...
We conducted the major part of the experiments with \gls{Fimex} using a file provided by the customer which covered and huge area, but at a lower resolution and only for current data, therefore it was not so big as other files in size (165 MB instead of 3-4 GB or more). This was chosen in order to reduce any possible scalability problem in the conversion process due to handling huge files.
...
The configuration file that we used to convert to Spherical Mercator projection is the following:
\begin{lstlisting}
[input]
file=ORIGPATH
...
In the parameters it is not needed to specify the projection from which to convert from, since \gls{Fimex} recognizes it automatically. Therefore, no changes of the command line string or configuration file are needed to convert from a \gls{Fimex} file in another format.

There have been multiple challenges that blocked or obstructed our work on the configuration of \gls{Fimex}. Specifically, the following challenges have been faced:
\begin{itemize}
...
Understanding \gls{Fimex} configuration files has been difficult, and required a considerable amount of work because of the poor documentation, and the almost total lack of relevant examples.
The only source of examples found is the \gls{Fimex} mailing list, but messages are not searchable, and difficult to consult since they are stored and available only one message at a page.
In the end, an example was extracted from the error log of a non-working solution on the \gls{Fimex} mailing list\footnote{http://lists.met.no/pipermail/fimex/2010-September/000013.html} .
...
This has been a big issue for the group. All the \gls{Fimex} generated \gls{netcdf}-files were lacking  the attribute grid\_mapping, but this attribute is needed by the Java NetCDF library, used by the \gls{back-end} part of the application. A lot of time has been spent figuring out the reason for this, and how to deal with the problem. 
...
\begin{figure}[!htb]
  \begin{center}
    ...
    \caption{The Mercator Projection Attribute in the \gls{netcdf} file generate by \gls{Fimex}, visualized with Panoply}
...
In the figure \ref{fig:Back-endArchitecture} is shown flow of the \gls{back-end}, how the modules are connected and short description what the module does.
...
When a request comes in, it is automatically routed by the webserver module to the correct Resource class. The Resource class parses the parameters supplied by the user, and talks to the \textit{\gls{netcdf} manager} module in order to obtain the relevant data.
...
The \textit{\gls{netcdf} indexer} keeps a database entry for each \gls{netcdf} file, containing information on what area is covered, at what times and at what level of detail. The \textit{\gls{netcdf} manager} module can use this to select the correct files to open in order to fulfill a request.
  ...
  Aside from various maintanance and indexing tables required by spatialite, the database contains one table for our \gls{netcdf} information, dataset\_XY\_time. This table contains the columns detailed in table \ref{tab:DatabaseColumns}. To set up this table, the following SQL statements are executed on a database set up for spatialite:
\begin{lstlisting}
CREATE TABLE dataset_XY_time (
    id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    filepath TEXT NOT NULL,
    gridsize INTEGER NOT NULL);

SELECT AddGeometryColumn('dataset_XY_time', 'coverage', 33333, 'POLYGON', 'XY', 1);

SELECT AddGeometryColumn('dataset_XY_time', 'time', 33333, 'LINESTRING', 'XY', 1);
\end{lstlisting}
...
\begin{figure}[!htb]
\begin{center}
\fbox{\includegraphics[width=300px]{img/indexer_datasets.png}}
...
\begin{figure}[!htb]
\begin{center}
\fbox{\includegraphics[width=300px]{img/indexer_request.png}}
...
The REST endpoints are gateway to the application as all communication flows through them and \gls{API} is defined by those endpoints. It is a collection of classes representing specific service that the application provides. When a user enters his request in the form of \gls{URL} into his \gls{HTTP} client (browser), the server parses this request and calls appropriate resource class to handle this request. The user can specify additional constraints in the form of query parameters.
...
To minimize boilerplate code, the \texttt{RsUtil} class containing common methods was created. This class contains methods for user input validation and for converting array of vectors to array of its magnitudes.
...
Otherwise, it then calls the relevant resource with the computed coordinates to obtain the image for that tile. Finally, it calls the cache to store the image for later use.
...
During the design of the \gls{front-end} interface, some inspiration has been taken from existing solutions like Google Maps and Bing maps. 
...
The team has taken the decision to use a map with OpenStreetMap background tile covering all the screen size. This is mainly because it does not require a license and is free to use.
...
This conversion is done on the box dragged end event, i.e. every time the user selects a new area on the screen.
...
The project task was broadly specified and gave us a the opportunity to take our work in many directions, and the possibility to develop a lot of features. While the customer in the end is looking for a commercial solution that includes all of these, it was clear from the start that the scope of our task could not be to make a full commercial product. The scope that was agreed with the customer is defined in section \ref{sec:AssignmentScope}. The reason for limiting the scope is that this would enable making what was actually developed during the project robust and efficient, being efficient enough to serve its purpose with very large pieces of data. Efficiency has been a problem in previous attempts, which made this prioritization one of extra importance.

As defined by Measurement of project effects in section \ref{sec:MeasurementProjEff}, a successful product is defined by successfully implementing all high and medium requirements, and passing all tests. Looking at the successfully implemented requirements, the project is behind this schedule. Several requirements have not been implemented, like handling overlapping areas (FR 1.5), automatically preparing files added to the system (FR 1.11), providing current direction data in an easily usable format (FR 1.20) and displaying it in \gls{front-end} (FR 2.8).

The challenges we have faced with getting the projections working has blocked many of these tasks, and stolen a lot of the time that was designated for these tasks. These challenges are discussed further in section \ref{sec:ProjectionChallenges}.

In the product requirements the performance of the system was described as general as "The requests must be fast enough to feel responsive for the user". It was not be possible to describe it more specifically, given the uncertainty of the final solution. The amount of data the system should store and process is very large, and even a normal response time can be a challenge. In the final product the response time is low. This is something we are quite satisfied with given that previous attempts have struggled on this point. This has been achieved by thorough consideration of solutions for bottlenecks and efficient use of existing tools and technology. \\

Overall the project has been robustly implemented despite not reaching all of its important functional requirements. We have also done a lot of work outside of what is included in the final solution as we have faced multiple challenges. A thorough report of the findings of the team is also part of what is handed to the customer, and this should be of good use to the customer. The final solution should be easy to utilize with detailed deploy guides and we have given much thought to work that can be done to further improve the solution.
...
A tool to select the area you want to query was implemented in the \gls{GUI}. It is toggled on and off by pressing a button in the \gls{GUI} or you can hold down the shift button on your keyboard while clicking and dragging on the map. The keyboard shortcut is not mentioned anywhere in the \gls{GUI}, and the user will probably not find this feature on his own. We therefore recommend that tool tips and hint are added to the \gls{GUI} so the user knows about all of its features. We can also use tooltips to minimize the \gls{GUI} and just show the explanation when moving the mouse over the field.

\subsubsection{Re-investigating Leaflet}
In our \gls{pre-study} we investigated Leaflet but choose to go with OpenLayers because of native \gls{WMTS}-support. After getting projections done correctly we were finally able to test the \gls{front-end}, and we found that missing support for image stretching in OpenLayers version 3 causes problems with getting the image overlay to cover the correct portions of the map. Towards the end of the project we found out that Leaflet is able to handle this specific problem and although there is no native \gls{WMTS}-support we have found that this can be added through plug-ins. 

A \gls{prototype} was made that used the existing \gls{front-end} solution with Leaflet replacing just OpenLayers-specific functionality. This prototype managed to achieve the existing \gls{front-end} functionality and to handle the problems that arose with image stretching.

Since this happened so late in the project lifecycle and the prototype is yet to be thoroughly tested, this is a point for re-investigation in the further work. It will have to be investigated if the \gls{WMTS}-support of Leaflet will be good enough, or if it is possible to implement \gls{WMTS} on OpenLayers, and avoid the problems we have faced with stretching the images.
...
To add this functionality it is necessary to modify resource classes, and the goal may be achieved in multiple ways. The easiest solution would be to add a new method to the resource, which will return raw data when requested. This would however not be an optimal solution. Another way is to modify the current method for getting the wanted data to distinguish between multiple output formats. Then it will have to be decided what is the right way of specifying the output format. It can be passed to the method as another argument, or it can be decided from the ACCEPT field in \gls{HTTP} request.

It is also important to decide a format in which the raw data will be coded. This format should be as compact as possible, but definitely lossless. Because of the current architecture of the application, it would be preferable if this data was transmitted as \gls{JSON} (or at least packed in \gls{JSON}).
...
A much better option is to register some kind of event listener that is triggered by the system whenever the event occurs. This implementation is very efficient, because the function is called only when needed, but requires a file system that has native support for those notifications. In Java there is a Watch Service \gls{API} in \texttt{java.nio.file} package. This package contains functions for registering event handlers and in case of file system without support for such notifications falls back to polling. More information about this \gls{API} and usage can be found at \url{https://blogs.oracle.com/thejavatutorials/entry/watching_a_directory_for_changes}.

\subsubsection{Allow user to specify resolution of output}
Currently the output resolution is hardcoded to \texttt{ImageRenderer} to have the shortest side of resulting images be 256 pixels. When handling a request all data at that time and depth is read, until it reaches the \texttt{ImageRenderer}, where it is scaled to final resolution. There should be a possibility for the user to specify the target resolution for the data he requests.
...
\texttt{NetCdfManager} has to be modified to create an array of the right size corresponding to the resolution, and fill it with the correct data. For reading data from a dataset with higher resolution, the \texttt{makeSubset} function allows to specify a stride to be used when getting data. For calculating the right stride for the given resolution, a dummy function was created \texttt{calculateStride} in \texttt{NetCdfManager}.

\subsubsection{Non-static ranges for image renderer}
Currently all images uses static minimum and maximum values suggested by the customer. The resulting color of every pixel is then decided by the distance from those two extremes. \\ \\
Moving away from these static scales can be useful in a couple of occations. The problem arises when the values in the netcdf-file range over a small portion of the static range these would be represented by simillar colors which could be difficult to distinguish. 

To solve this we can use a dynamic range based on the minimum and maximum values found in the selected area. When requesting multiple images and joining them together it is undesirable to have different scales for every picture. Together with the stateless HTTP protocol using this in a map service with multiple tiles and requests will create new challenges. It could however be a very useful for single image calls outside of \gls{WMTS}, when image contrast has to be maximized.

The other option is to let the user define the range as decribed in \ref{sec:LegendColorMap} on page \pageref{sec:LegendColorMap}. This way the \gls{front-end} could request the map tiles scale selected by the user and get a uniform representation across the tiles. \\ \\
To add these functionalities \texttt{ImageRenderer} has to be modified to allow specifying the minimum and maximum values or request it to dynamically calculate these.

\subsubsection{Further projection functionality}
Getting the projections done correctly has been a big difficulty in the project development, and it will therefore need a lot of further work to serve all the functionality that is wanted, as well as to minimize the workload of the rest of the \gls{back-end} and human personnel.
%Fimex automation %REF/GLS fimex?
\paragraph{\gls{Fimex} automation}
...
One of the main bottlenecks for this kind of service is disk reads. Since the data sets are so big, you can only have a fraction of the data in memory. This means that every request likely results in at least one disk read. In order to handle a large traffic volume, one should consider load balancing the service. This means having multiple servers serving the data from a single url.
...
Adding caching only makes sense for the WMTS service, because the REST \gls{API} has no real notion of reusability - every request is in practice arbitrary. However, for \gls{WMTS} requests, every tile coordinate is static and caching should yield a real performance improvement - especially under heavy load.
...
Upon tile request, check if the tile exists in cache. If it does, just serve it. If it does not exists, serve it and then store it in the cache.
...
\subsection{Area}
\label{subsect:api-area}

All features support \texttt{area} subservice, which provides information about given feature in given area, currently in a form of \gls{PNG} image with the shorter side set to 256 px and longer side scaled to keep the ratio of the requested data. In case of error, \gls{HTTP} status is set accordingly and \gls{JSON} specifying the error is returned. Returned \gls{JSON} then can contain those fields:
\begin{description}
...
\begin{description}
	...
	\item[depth] Specifies depth in meters. This parameter is ignored in depth service, otherwise it is a compulsory parameter.
	\item[time] Specifies time in ISO-8601. This parameter is ignored in depth service, otherwise it is a compulsory parameter.
\end{description}
...
The salinity and temperature features also support \texttt{profile} subservice, which provides vertical profile at the given coordinates. This resource returns requested data as a JSON described below, or, in case of error, the resource returns JSON describing the situation in the same format as in subsection \ref{subsect:api-area}.

\begin{lstlisting}[caption=JSON object returned by vertical profile resource, label=lst:api-profile-json, frame=single]
{
    "feature": {feature name},
    "profile": [Array of values in depth],
    "missingValuePlaceholder": -1,
    "location": {
        "latitude": {latitude},
        "longitude":{longitude},
        "time":     {time}
...
To specify data you are interested in, the subservice takes this parameters:
\begin{description}
	\item[\underline{lat}] Latitude of the source data point you are interested in. This parameter is compulsory.
	\item[\underline{lon}] Longitude of the source data point you are interested in. This parameter is compulsory.
	\item[\underline{time}] Specifies time in ISO-8601. This parameter is compulsory.
\end{description}

\section{Overview}

In figure \ref{fig:rest_api_tree} you can see a tree describing REST \gls{API}. The root of the tree represents the root of the application, and \gls{URL}s are represented by paths from root to leaf nodes.
...
In this example we will try to retrieve depth information, but will not specify any parameters.
...
\end{description}
\section{WMTS}
This section describes how the \gls{WMTS} API would behave. We did not manage to implement the WMTS specifications in time.

Since WMTS is designed as a Machine to Machine protocol, it s relatively simple in structure. It consists of two resources:

\textit{Capabilities} and \textit{Tile}. 
\subsection{Capabilities resource} 
The Capabilities resource hosts an XML document that describes the layout of the tile matrix set, as described in \ref{sec:WMTS}. The client uses this to inform itself of the tile layout, in order to lay out the tile images correctly on its background map.  The parameters of the XML document are computed from the dataset.
...
By running the following command gulp will watch all the source files in the \emph{src} folder. If any changes are made, the necessary compiling will be done automatically and the relevant files will be copied into its respective folders in the \emph{dist} folder.

\begin{lstlisting}
$ gulp
\end{lstlisting}

Note: Since the \gls{URL} address of the \gls{back-end} server may change according to the machine on which it is deployed, changing it may be needed front-end as well. This can be done in the file \textit{src/js/script.js} in the first line of the \textit{buildurl()} function.
...
\paragraph{Installation of \gls{Fimex}}
\gls{Fimex} library can be installed on an Ubuntu 14.04 LTS Operating System by running the following commands in a terminal window:
\begin{lstlisting}
...
Afterwards \gls{Fimex} is ready in the command line. To check if the installation is correct it is possible to run:
\begin{lstlisting}
fimex --version
\end{lstlisting}
...
\chapter{Templates}
In the \gls{course compendium}, the team is asked to create templates for internal documents. The team identified three types of documents they would need templates for:
