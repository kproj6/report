...
In our prestudy we investigated Leaflet but choose to go with OpenLayers because of native \gls{WMTS}-support. After getting projections done correctly were finally able to test the \gls{front-end}, and we found that missing support for image stretching in OpenLayers version 3 causes problems with getting the image overlay to cover the correct portions of the map. Towards the end of the project we found out that Leaflet can be able to handle this specific problem and although there is no native \gls{WMTS}-support we have found that this can be added through plug-ins. 

A prototype was made that used the existing \gls{front-end} solution with Leaflet replacing just OpenLayers-specific functionality. This prototype managed to achieve the existing \gls{front-end} functionality and to handle the problems that arose with image stretching.

Since this happened so late in the project lifecycle and the prototype is jet to be thoroughly tested, this is a point to be done for re-investigation in the further. We will have to investigate if the \gls{WMTS}-support of Leaflet will be good enough. Or if it is possible to implement \gls{WMTS} on OpenLayers and not avoid the problems we have faced with streatching the images.
