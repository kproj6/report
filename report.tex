...
A set of possible risks factors that may cause trouble for the team in completing the project has been identified. Each risk has an associated probability and consequence if that specific event occurs. The probability means how likely is that the risk will take effect, and the consequence means how severe the effects would be if the risk occurs. Both probability and consequence are assigned within the categories low (L), medium (M) and high (H), creating a so called “risk matrix”.
For each risk we have defined a strategy for how it should be reduced (by reducing the probability, the consequence, or both) or completely avoided. 
