...
With this in mind, the group feels that the only realistic solution for the \gls{front-end} is to develop a custom system from scratch. Specifically we think using the OpenLayers JavaScript library will beneficial in this project.
...
The salinity and temperature features support also \texttt{profile} subservice, which provides vertical profile at given coordinates. This resource returns requested data as a JSON described below, or, in case of error, resource returns JSON describing situation in the same format as in subsection \ref{subsect:api-area}.
